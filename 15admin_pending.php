        ...
        <!-- NOTIFICATION FOR ADD/EDIT/DELETE -->
         <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . ' 
                <a href="../IMISS-System/15admin_pending.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if (isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . ' 
                <a href="../IMISS-System/15admin_pending.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../IMISS-System/15admin_pending.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../IMISS-System/15admin_pending.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP -->
            ...
            <table>
                ...
                <tbody> 
                        ...
                        $sql =  "SELECT  CONCAT(MONTH(a.date_requested), '-', YEAR(a.date_requested), '-', a.id) AS full_id, 
                                    ...
                                    a.id AS take_id, a.end_user_id, a.status, a.ict_equipment_code, a.description, a.type_of_service, a.ict_component, 
                        ...
                        ?>
                    <tr class="table_row">
                        <td><a href="../IMISS-System/PHP-Dashboard-Imiss/pending_request.php?id=<?php echo $row["take_id"] ?>"><?php echo $row["full_id"] ?></a></td>
