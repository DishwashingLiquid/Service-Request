...
<head>
    ...
    <meta http-equiv="refresh" content="320;url=2reception.php"/>
    ...
    <div class="paper">  
        ...
        <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . ' 
                <a href="../IMISS-System/2reception.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if (isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . ' 
                <a href="../IMISS-System/2reception.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../IMISS-System/2reception.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../IMISS-System/2reception.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP -->
        <form action="../IMISS-System/PHP-Reception/actions.php" method="POST"> <!-- used form to add, delete and edit with checkbox work -->
        <div class="controls">
        ...
        <table>
            ...
            <tbody>
                    <?php
                    $sql = "SELECT CONCAT(id_name, id) AS full_id, id, reception_fname, reception_mname, reception_lname, username, password, updated_by FROM `reception-list`
                            ORDER BY id ASC";
                ...
                <tr class="table_row">
                    ...
                    <td><?php echo $row["reception_fname"] ?></td>
                    <td><?php echo $row["reception_mname"] ?></td>
                    <td><?php echo $row["reception_lname"] ?></td>
