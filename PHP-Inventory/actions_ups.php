<?php
    ...
    if(isset($_POST["add_submit"])) { 
        ...
        $sql = "INSERT INTO `equipment-list-ups`
                    (`area`, `brand`, `model`, `type`, `no_of_kva`, `updated_by`)
                VALUES
                    ('$area', '$brand', '$model', '$type', '$no_of_kva', '$updated_by')
                ";
    ...
    if($result) {
        header("Location: ../../IMISS-System/PHP-Inventory/actions_ups.php?scs=Added successfully.");
    } else {
    ...
    <div class="paper">
        ...
        <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . '
                <a href="../../IMISS-System/PHP-Inventory/actions_ups.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if(isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . '
                <a href="../../IMISS-System/PHP-Inventory/actions_ups.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_ups.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_ups.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP -->
            ...
            <div class="all_forms" style="margin-top: 0vh">
                <div class="inventory_spec">
                    <?php
                        ...
                        $sql = "SELECT MAX(id) as max_id FROM `equipment-list-ups`";
                        $result = mysqli_query($conn, $sql);
                        $row = mysqli_fetch_assoc($result);
                        $next_id = $row['max_id'] + 1; /* to get the next auto-id */
                    ?>
                    <div class="input_field top_id">
                        <input type="text" name="next_id" id="next_id" value="<?php echo 'UPS-', $today, '-', $next_id ?>" disabled>
                    ...
                    <div class="input_field">
                        <input type="text" name="no_of_kva" id="no_of_kva" required>
        ...
        <form action="" method="POST">
            ...
            <?php 
                ...
                <?php } ?>
        <!-- TABLE DATA -->
        <div class="container" style="height: 56.5vh;">
        <table>
            ...
            <tbody>
                    <?php
                        $sql = "SELECT CONCAT(id_name, '-', id_year, '-', id) AS full_id, 
                                id, area, brand, model, type, no_of_kva, status, updated_by
                    ...
                    ?>
                <tr class="table_row"> 
                    <td><a href="../../IMISS-System/PHP-Inventory/functions_ups.php?id=<?php echo $row['id']?>"><?php echo $row["full_id"]?></a></td>
                    <td><?php echo $row["area"]?></td>
                    <td><?php echo $row["brand"]?></td>
                    <td><?php echo $row["model"]?></td>
                    <td><?php echo $row["type"]?></td>
                    <td><?php echo $row["no_of_kva"]?></td>
        ...
        <div class="table_control">
            <?php
                $sql = "SELECT * FROM `equipment-list-ups`
                        WHERE status = 'Active'";
