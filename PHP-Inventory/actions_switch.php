<?php
    ...
    if(isset($_POST["add_submit"])) {
        ...
        $sql = "INSERT INTO `equipment-list-switch`
                    (`area`, `brand`, `model`, `no_of_ports`, `updated_by`)
                VALUES
                    ('$area', '$brand', '$model', '$no_of_ports', '$updated_by')";
        $result = mysqli_query($conn, $sql);

    if($result) {
        header("Location: ../../IMISS-System/PHP-Inventory/actions_switch.php?scs=Added successfully.");
    } else {
    ...
    <div class="paper">
        ...
        <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . '
                <a href="../../IMISS-System/PHP-Inventory/actions_switch.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if(isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . '
                <a href="../../IMISS-System/PHP-Inventory/actions_switch.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_switch.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_switch.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP  -->
        ...
        <form action="" method="POST" autocomplete="off">
            <div class="all_forms" style="margin-top: 0vh">
                <div class="inventory_spec">
                    <?php
                        $today = date("Y"); /* to get the next id_year */
                    ...
                    <div class="input_field top_id">
                        <input type="text" name="next_id" id="next_id" value="<?php echo 'SWT-', $today, '-', $next_id ?>" disabled>
                    ...
                    <div class="input_field">
                        <input type="text" name="no_of_ports" id="no_of_ports" required>
        ...
        <table>
            ...
            <tr> 
                ...
                <th scope="col">number of ports</th>
            ...
            <tbody>
                    <?php
                        $sql = "SELECT CONCAT(id_name, '-', id_year, '-', id) AS full_id, 
                                id, area, brand, model, no_of_ports, status, updated_by
                                FROM `equipment-list-switch`
                                ORDER BY id DESC";
                        $result = mysqli_query($conn, $sql);
                    ...
                    ?>
                <tr class="table_row"> 
                    <td><a href="../../IMISS-System/PHP-Inventory/functions_switch.php?id=<?php echo $row['id'] ?>"><?php echo $row["full_id"]?></a></td>
                    <td><?php echo $row["area"]?></td>
                    <td><?php echo $row["brand"]?></td>
                    <td><?php echo $row["model"]?></td>
                    <td><?php echo $row["no_of_ports"]?></td>
        ...
        <div class="table_control">
            <?php
                $sql = "SELECT * FROM `equipment-list-switch`
                        WHERE status = 'Active'";
