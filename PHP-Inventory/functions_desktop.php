<?php
    ...
    if(! isset($_SESSION["log_admin"]) && ! isset($_SESSION["log_reception"]) && ! isset($_SESSION["log_superadmin"])) {
     ...
     if(isset($_POST["edit_submit"])) { 
        ...
        $sql = "UPDATE `equipment-list-desktop` SET 
                `area` = '$area',
                `brand` = '$brand',
                `model` = '$model',
                `ms_office` = '$ms_office',
        ...
        if($result) {
            header("Location: ../../IMISS-System/PHP-Inventory/actions_desktop.php?scs=Updated successfully.");
        } else {
...
<body>
    ...
    <div class="paper">
        ...
        <div class="all_forms"> 
            <div> <!-- div for edit --> 
                <h3>Edit Desktop Inventory</h3> 
                <form action="" method="post" autocomplete="off">  
                    <?php 
                        $id = $_GET["id"];   
                        $sql = "SELECT CONCAT(id_name, '-', id_year, '-', id) AS full_id, id, area, brand, model,
                                        ms_office, ups, os, status
                                FROM `equipment-list-desktop` WHERE id = $id";
                        $result = mysqli_query($conn, $sql);
                    ...
                    <div class="input_field">
                        <input type="text" name="ms_office" id="ms_office" value="<?php echo $row['ms_office']?>" required>
                        <label for="ms_office">MS Office:</label>
                    </div>
                    <div class="input_field">
                        <input type="text" name="ups" id="ups" value="<?php echo $row['ups']?>" required>
                        <label for="ups">UPS:</label>
                    </div>
                    <div class="input_field">
                        <input type="text" name="os" id="os" value="<?php echo $row['os']?>" required>
                    ...
                    <a href="../../IMISS-System/PHP-Inventory/actions_desktop.php" class="button_control">Cancel</a> 
                </form>
            </div>  
