<?php
    ...
    if(isset($_POST["add_submit"])) {
        ...
        $sql = "INSERT INTO `equipment-list-printer`
                    (`area`, `brand`, `model`, `type`, `ink_type`, `ink_code`, `updated_by`)
                VALUES
                    ('$area', '$brand', '$model', '$type', '$ink_type', '$ink_code', '$updated_by')
                ";
    ...
    if($result) {
        header("Location: ../../IMISS-System/PHP-Inventory/actions_printer.php?scs=Added successfully.");
    } else {
    ...
    <div class="paper">
        ...
        <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . '
                <a href="../../IMISS-System/PHP-Inventory/actions_printer.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if(isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . '
                <a href="../../IMISS-System/PHP-Inventory/actions_printer.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_printer.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../../IMISS-System/PHP-Inventory/actions_printer.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP -->
            ...
            <div class="all_forms" style="margin-top: 0vh">
                <div class="inventory_spec">
                    ...
                    <div class="input_field top_id">
                        <input type="text" name="next_id" id="next_id" value="<?php echo 'PRT-', $today, '-', $next_id ?>" disabled>
        ...
        <table>
            ...
            <tr> 
                ...
                <th scope="col">ink type</th>
                <th scope="col">ink code</th> 
            ...
            <tbody>
                    <?php
                        $sql = "SELECT CONCAT(id_name, '-', id_year, '-', id) AS full_id, 
                                id, area, brand, model, type, ink_type, ink_code, status, updated_by
                                FROM `equipment-list-printer`
                                ORDER BY id DESC";
                        $result = mysqli_query($conn, $sql);
                    ...
                    ?>   
                <tr class="table_row"> 
                    <td><a href="../../IMISS-System/PHP-Inventory/functions_printer.php?id=<?php echo $row['id']?>"><?php echo $row["full_id"]?></a></td>
                    <td><?php echo $row["area"]?></td>
                    <td><?php echo $row["brand"]?></td>
                    <td><?php echo $row["model"]?></td>
                    <td><?php echo $row["type"]?></td>
                    <td><?php echo $row["ink_type"]?></td>
                    <td><?php echo $row["ink_code"]?></td>
        ...
        <div class="table_control">
            <?php
                $sql = "SELECT * FROM `equipment-list-printer`
                        WHERE status = 'Active'";
