<?php 
    ...
    if(isset($_POST["add_submit"])) { 
        ...
        $sql = "INSERT INTO `equipment-list-desktop`
                    (`area`, `brand`, `model`, `ms_office`, `ups`, `os`, `updated_by`)
                VALUES
                    ('$area', '$brand', '$model', '$ms_office', '$ups', '$os', '$updated_by')";
        ...
        if($result) {
            header("Location: ../../IMISS-System/PHP-Inventory/actions_desktop.php?scs=Added successfully.");
        } else {
...
<body>
    ...
    <div class="section">
    <div class="paper">
        <div class="titlebar">desktop</div> 
           ...
           <?php
                if (isset($_GET["dng"])) {
                    ...
                    ' . $dng . ' 
                    <a href="../../IMISS-System/PHP-Inventory/actions_desktop.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                    </div>'; 
                } else if (isset($_GET["scs"])) {
                    $scs = $_GET["scs"];
                    echo '<div class="success_msg" role="alert">
                    ' . $scs . ' 
                    <a href="../../IMISS-System/PHP-Inventory/actions_desktop.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                    </div>';
                } else if (isset($_GET["info"])) {
                    $info = $_GET["info"];
                    echo '<div class="info_msg" role="alert">
                    ' . $info . ' 
                    <a href="../../IMISS-System/PHP-Inventory/actions_desktop.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                    </div>'; 
                } else if(isset($_GET["wng"])) {
                    $wng = $_GET["wng"];
                    echo '<div class="warning_msg" role="alert">
                    ' . $wng . ' 
                    <a href="../../IMISS-System/PHP-Inventory/actions_desktop.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                    </div>';
                }
            ?>
            <!-- CONTROLS ON THE TOP --> 
                ...
                <div class="all_forms" style="margin-top: 0vh;"> 
                    <div class="inventory_spec">
                        <?php 
                            ...
                            $sql = "SELECT MAX(id) as max_id FROM `equipment-list-desktop`";
                            $result = mysqli_query($conn, $sql);
                            $row = mysqli_fetch_assoc($result);
                            $next_id = $row['max_id'] + 1; /* to get next auto-id */
                        ?>
                        <div class="input_field top_id">
                            <input type="text" name="next_id" id="next_id" value="<?php echo 'PC-', $today, '-', $next_id ?>" disabled>
                        ...
                        <div class="input_field">
                            <input type="text" name="ups" id="ups" required>
                ...
                <tbody>
                        <?php
                            $sql = "SELECT CONCAT(id_name, '-', id_year, '-', id) AS full_id,
                                    id, area, brand, model, ms_office, ups, os, status, updated_by
                        ...
                        ?>
                    <tr class="table_row"> 
                        <td><a href="../../IMISS-System/PHP-Inventory/functions_desktop.php?id=<?php echo $row['id'] ?>"><?php echo $row["full_id"]?></a></td>
        ...
        <div class="table_control">
            <?php
                $sql = "SELECT * FROM `equipment-list-desktop`
                        WHERE status = 'Active'";
