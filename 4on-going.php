...
<head>
    ...
    <meta http-equiv="refresh" content="320;url=4on-going.php"/>
    ...
    <div class="paper">  
        ...
        <?php
            if (isset($_GET["dng"])) {
                ...
                ' . $dng . ' 
                <a href="../IMISS-System/4on-going.php" id="danger_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if (isset($_GET["scs"])) {
                $scs = $_GET["scs"];
                echo '<div class="success_msg" role="alert">
                ' . $scs . ' 
                <a href="../IMISS-System/4on-going.php" id="success_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            } else if (isset($_GET["info"])) {
                $info = $_GET["info"];
                echo '<div class="info_msg" role="alert">
                ' . $info . ' 
                <a href="../IMISS-System/4on-going.php" id="info_close"><i class="fa-solid fa-xmark"></i></a>
                </div>'; 
            } else if(isset($_GET["wng"])) {
                $wng = $_GET["wng"];
                echo '<div class="warning_msg" role="alert">
                ' . $wng . ' 
                <a href="../IMISS-System/4on-going.php" id="warning_close"><i class="fa-solid fa-xmark"></i></a>
                </div>';
            }
        ?>
        <!-- CONTROLS ON THE TOP -->
        <form action="../IMISS-System/PHP-On-Going/actions.php" method="POST"> <!-- used form to add, delete and edit with checkbox work -->
        <div class="controls">    
        <button type="submit" name="create_button" id="create_button" class="button_control">Create New</button> 
           ...
           <!--  <div class="search_field">
                ...
                <div class="inside_search"> --> <!-- used for the button to go inside the search input -->
                 <!--    <button  name="search_button" id="search_button" class="button_control">Search</button> 
                </div>
            </div> -->
        </div>
        <!-- TABLE DATA -->
            ...
            <tbody>
                    <?php 
                        $sql =  "SELECT CONCAT(MONTH(a.date_requested), '-', YEAR(a.date_requested), '-', a.id) AS full_id, 
                                    ...
                                    a.id AS take_id, a.ict_equipment_code, a.description, a.end_user_id, a.admin_id, a.status, a.updated_by,
                                ...
                                WHERE a.status != 'Pending' && a.status!= 'Resolved'
                ...
                <tr class="table_row"> 
                    ...
                    <td>
                        <?php 
                           ...
                           if(empty($admin_id)) { ?>
                                <a href="../IMISS-System/PHP-On-Going/assign_on-going.php?id=<?php echo $row["take_id"] ?>" class="button_control button_action">Assign To</a> 
                        <?php 
                        } else { ?>
                                <a href="../IMISS-System/PHP-On-Going/assign_on-going.php?id=<?php echo $row["take_id"] ?>" class="button_control button_action">Re-Assign To</a> 
                        <?php
                        } ?> 
                    </td>
