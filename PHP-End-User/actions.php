...
<body>
    ...
    <div class="paper">
        ...
        <div> <!-- div for add -->
            ...
            <form action="../../IMISS-System/PHP-End-User/actions_function.php" method="post" autocomplete="off">
                ...
                <a href="../../IMISS-System/3end-user.php" class="button_control">Cancel</a>
            </form>
        </div>
        <?php } elseif (isset($_POST["delete_button"])) { ?>
        <div> <!-- div for delete --> 
            <h3>Delete End-User</h3> 
            <form action="../../IMISS-System/PHP-End-User/actions_function.php" method="post" autocomplete="off">  
                <?php  
            ...
            <form action="../../IMISS-System/PHP-End-User/actions_function.php" method="post" autocomplete="off"> 
                <?php
                    ...
                    $sql = "SELECT CONCAT(id_name, $extract_id) AS full_id, 
                            division, area, end_user_fname, end_user_mname, end_user_lname, username, password, contact_no, email
                            FROM `end-user-list` WHERE id = $extract_id LIMIT 1";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);
                ?>
                <div class="input_field no_display"><!-- this will let the id still submit  -->
                ...
                <div class="input_field">
                    <input type="text" name="area" value="<?php echo $row['area'] ?>" required>
                    <label for="area">Area/Section/Unit:</label> 
                </div>
                <div class="input_field">
                    <input type="text" name="end_user_fname" value="<?php echo $row['end_user_fname'] ?>" required>
                    <label for="end_user_fname">First Name:</label>
                </div>
                <div class="input_field">
                    <input type="text" name="end_user_mname" value="<?php echo $row['end_user_mname'] ?>" required>
                    <label for="end_user_mname">Middle Name:</label>
                </div>
                <div class="input_field">
                    <input type="text" name="end_user_lname" value="<?php echo $row['end_user_lname'] ?>" required>
                    <label for="end_user_lname">Last Name:</label>
                </div>
                <div class="input_field">
