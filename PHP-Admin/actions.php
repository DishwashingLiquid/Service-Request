...
<body>
    ...
    <div class="paper"> 
        ...
        <div> <!-- div for add -->
            <h3>Add New IMISS Staff</h3> 
            <form action="../../IMISS-System/PHP-Admin/actions_function.php" method="post" autocomplete="off">  
                <div class="input_field"> 
                    <input type="text" name="admin_fname" id="admin_fname" required> 
                    <label for="admin_fname">First Name:</label>
                </div>
                <div class="input_field">
                    <input type="text" name="admin_mname" id="admin_mname" required>
            ...
            <form action="../../IMISS-System/PHP-Admin/actions_function.php" method="post" autocomplete="off">  
                <?php 
                    ...
                    $sql = "SELECT CONCAT(id_name, $extract_id) AS full_id,
                            admin_fname, admin_mname, admin_lname, username, password
                            FROM `admin-list` WHERE id = $extract_id LIMIT 1";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);
                ?>  
                <div class="input_field no_display"> <!-- this will let the id still submit  -->
                ...
                <div class="input_field">
                    <input type="text" name="admin_fname" value="<?php echo $row['admin_fname'] ?>" required>
                    <label for="admin_fname">First Name:</label>
                    </div>
                <div class="input_field">
                    <input type="text" name="admin_mname" value="<?php echo $row['admin_mname'] ?>" required>
                    <label for="admin_mname">Middle Name:</label>
                </div>
                <div class="input_field">
                    <input type="text" name="admin_lname" value="<?php echo $row['admin_lname'] ?>" required>
                    <label for="admin_lname">Last Name:</label>
                </div>
                <div class="input_field">
